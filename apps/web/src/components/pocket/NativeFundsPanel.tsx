...
export function NativeFundsPanel({ pocketId, nativeBalance, open, onClose, onSuccess }: NativeFundsPanelProps) {
  ...
  // nativeBalance is in lamports; display in SOL
  const balanceSol = (Number(nativeBalance) / 1e9).toFixed(6)

  async function handleSubmit() {
    if (!amount || isNaN(Number(amount)) || Number(amount) <= 0) { setError('Enter a valid amount'); return }
    if (tab === 'withdraw' && !toAddress.trim()) { setError('Enter a destination address'); return }
    setError(''); setSuccess(''); setLoading(true)
    try {
      if (tab === 'deposit') {
        await api.deposit(pocketId, amount)
        setSuccess(`Deposited ${amount} SOL to your pocket`)
      } else {
        await api.withdraw(pocketId, amount, toAddress)
        setSuccess(`Withdrawn ${amount} SOL to ${toAddress.slice(0, 8)}...`)
      }
  ...
  return (
    <Modal open={open} onClose={onClose} title="Native SOL funds">
      <div>
        <div className="bg-zinc-800 rounded-xl p-4 mb-5 text-center">
          <p className="text-xs text-zinc-500 mb-1">Native balance</p>
          <p className="text-2xl font-bold text-white">{balanceSol} <span className="text-green-400">SOL</span></p>
        ...
        <div className="flex gap-1 mb-5 bg-zinc-900 p-1 rounded-xl">
          {(['deposit', 'withdraw'] as const).map((t) => (
            <button key={t} onClick={() => { setTab(t); setError(''); setSuccess('') }}
              className={cn('flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors capitalize',
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input className="input" type="number" step="0.001" min="0" placeholder="0.0"
              value={amount} onChange={(e) => setAmount(e.target.value)} />
          </div>

          {tab === 'withdraw' && (
            <div>
              <label className="block text-sm text-zinc-400 mb-1.5">Destination Solana address</label>
          ...
          {tab === 'deposit' && (
            <div className="bg-zinc-800/50 rounded-xl p-3 text-xs text-zinc-500 space-y-1">
              <p>• SOL is held natively in your Clutch pocket</p>
              <p>• The AI agent uses this balance for automatic payments</p>
              <p>• Withdraw anytime to any Solana address</p>
            </div>
          )}
